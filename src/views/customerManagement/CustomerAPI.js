...
export const getCustomerDataByID = async (mobileNumber) => {
  try {
    const response = await axios.get(`${BASE_URL}/${CustomerDataByID}/${mobileNumber}`)
    return response.data
  } catch (error) {
    handleError(error)
  }
...
export const addAddressData = async (mobileNumber, customerData) => {
  try {
    ...
    const response = await axios.post(`${BASE_URL}/${AddAddress}?mobileNumber=${mobileNumber}`, customerData, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
...
export const deleteAppointment = async (appointmentId,serviceId) => {
  try {
    ...
    const response = await axios.delete(`${BASE_URL}/${deleteAppointments}/${ appointmentId }/${serviceId}`, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
