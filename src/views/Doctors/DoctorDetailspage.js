...
const DoctorDetailsPage = () => {
  ...
  const handleDeleteToggleE = async (id) => {
    ...
    if (isDeleted) {
      navigate('/doctor')
      toast.success(isDeleted || 'Doctor Deleted successfully')
    } else {
  ...
  const handleAddSlot = async () => {
    ...
    try {
      const res = await axios.post(`${BASE_URL}/doctorId/${doctorData.doctorId}/slots`, payload)
  ...
  const slotsForSelectedDate =
    allSlots.find((slotData) => slotData.date === selectedDate)?.availableSlots || []
  ...
  const fetchSlots = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/doctorId/${doctorData.doctorId}/getDoctorslots`)
        ...
        <CCardBody>
          ...
          <CTabContent>
            ...
            <CTabPane visible={activeKey === 3} className="pt-3">
              {doctorRatings?.comments?.length > 0 ? (
                <div className="px-3">
                  <CRow>
                    <CCol md={6}>
                      <h5 className="mb-4">
                    ...
                    <CCol md={6}>
                      <h5 className="mb-4">
                        ...
                        Overall Rating: ⭐<strong>{doctorRatings.overall}</strong>
                      </h5>
                    </CCol>
                  </CRow>

                  {doctorRatings.comments.map((comment, index) => {
                    const initials = (comment.userName || comment.userId).slice(0, 2).toUpperCase()
                      ...
                      >
                        <div className="d-flex">
                          ...
                          <div className="flex-grow-1">
                            <div className="d-flex justify-content-between align-items-center">
                              <div>
                                <h6 className="mb-0">{comment.userName || comment.userId}</h6>
                                <small className="text-muted">{timeAgo}</small>
                              </div>
                              <div className="  fw-bold">
                                ⭐{comment.rating} <i className="fa fa-star" />
                              </div>
                            </div>
                        ...
                        {comment.replies?.length > 0 && (
                          <div className="mt-3 ps-4 border-start border-2 border-secondary">
                            <h6 className="text-secondary">Replies</h6>
                            {comment.replies.map((reply, rIdx) => {
                              const replyInitial = (reply.userName || reply.userId)
                                .slice(0, 2)
                                .toUpperCase()
                              const replyTime = formatTimeAgo(reply.createAt)
                              return (
                                <div key={rIdx} className="d-flex mt-3">
                                  <div
                                  ...
                                  <div>
                                    <div className="d-flex justify-content-between align-items-center">
                                      ...
                                      <small className="text-muted">{replyTime}</small>
                                    </div>
              ...
              <CTable striped bordered hover>
                <CTableHead>
                  <CTableRow>
                    ...
                    <CTableHeaderCell>Sub Services</CTableHeaderCell>
                  </CTableRow>
                </CTableHead>
                <CTableBody>
                  {doctorData?.category?.map((cat, index) => (
                    <CTableRow key={index}>
                      ...
                      <CTableDataCell>{cat.categoryName}</CTableDataCell>
                      <CTableDataCell>
                        {doctorData?.service?.map((srv) => srv.serviceName).join(', ')}
                      </CTableDataCell>
                      <CTableDataCell>
                        {doctorData?.subSerives?.map((sub) => sub.subServiceName).join(', ')}
                      </CTableDataCell>
                    </CTableRow>
                  ))}
                </CTableBody>
